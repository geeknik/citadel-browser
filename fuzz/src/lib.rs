...
pub mod security {
    ...
    impl MaliciousPayload {
        ...
        fn apply_evasion_technique(&self, payload: &str, technique: &EvasionTechnique) -> String {
            match technique {
                EvasionTechnique::WhitespaceVariation => payload.replace(" ", "\t\n\r "),
                EvasionTechnique::CommentInjection => payload.replace("<", "<!----><<"),
                ...
                EvasionTechnique::NullByteInjection => payload.replace("<", "<\0"),
                EvasionTechnique::UnicodeNormalization => payload.replace("a", "\u{0061}"),
...
fn base64_encode(input: &str) -> String {
    ...
    for chunk in bytes.chunks(3) {
        ...
        result.push(if chunk.len() > 1 {
            chars.chars().nth(((b >> 6) & 63) as usize).unwrap()
        } else {
            '='
        });
        result.push(if chunk.len() > 2 {
            chars.chars().nth((b & 63) as usize).unwrap()
        } else {
            '='
        });
    }
