use crate::{PagePermissions, ZkVmError, ZkVmResult};
...
impl ExecutorState {
    ...
    fn push_stack(&mut self, _value: u32) -> ZkVmResult<()> {
        ...
        self.sp = self
            .sp
            .checked_sub(4)
            .ok_or_else(|| ZkVmError::ExecutionError("Stack pointer underflow".into()))?;
    ...
    fn pop_stack(&mut self) -> ZkVmResult<u32> {
        ...
        self.sp = self
            .sp
            .checked_add(4)
            .ok_or_else(|| ZkVmError::ExecutionError("Stack pointer overflow".into()))?;
...
impl MemorySegment {
    ...
    fn new(base: u64, size: usize, permissions: PagePermissions, shared: bool) -> ZkVmResult<Self> {
        ...
        if size < MIN_SEGMENT_SIZE {
            return Err(ZkVmError::MemoryError(format!(
                "Segment size {} too small, minimum is {}",
        ...
        let end_address = base.checked_add(size as u64).ok_or_else(|| {
            ZkVmError::MemoryError("Integer overflow in segment end address calculation".into())
        })?;
    ...
    fn end_address(&self) -> ZkVmResult<u64> {
        self.base.checked_add(self.size as u64).ok_or_else(|| {
            ZkVmError::MemoryError("Integer overflow in segment end address calculation".into())
        })
    }
...
impl Executor {
    ...
    pub fn new(memory_limit: usize) -> ZkVmResult<Self> {
        ...
        if memory_limit > MAX_SEGMENT_SIZE * 100 {
            // Allow up to 100GB total
            return Err(ZkVmError::MemoryError(
    ...
    pub fn allocate_segment(
        ...
        if size == 0 {
            return Err(ZkVmError::MemoryError(
                "Cannot allocate zero-sized segment".into(),
            ));
        }
        if size > MAX_SEGMENT_SIZE {
            return Err(ZkVmError::MemoryError(format!(
        ...
        let new_allocated = address_space.allocated.checked_add(size).ok_or_else(|| {
            ZkVmError::MemoryError("Integer overflow in total memory calculation".into())
        })?;
    ...
    fn find_free_address_range(
        ...
        let check_fit = |candidate_base: u64| -> ZkVmResult<bool> {
            // Check for integer overflow in end address calculation
            let end_address = candidate_base.checked_add(size_u64).ok_or_else(|| {
                ZkVmError::MemoryError("Integer overflow in address range calculation".into())
            })?;
        ...
        for (_, segment) in &address_space.segments {
            ...
            let required_end = base.checked_add(size_u64).ok_or_else(|| {
                ZkVmError::MemoryError("Integer overflow in address calculation".into())
            })?;
            ...
            let aligned_base =
                (segment_end + MIN_SEGMENT_SIZE as u64 - 1) & !(MIN_SEGMENT_SIZE as u64 - 1);
    ...
    fn fetch_instruction(&self) -> ZkVmResult<u32> {
        ...
        if let Some((_, segment)) = address_space.segments.range(..=pc).next_back() {
            ...
            if segment.contains_address(pc)? {
                ...
                if !segment.permissions.execute {
                    return Err(ZkVmError::MemoryError(format!(
                        "No execute permission at address 0x{:x}",
                        pc
                    )));
                }

                // Ensure we can read a full 4-byte instruction without overflow
                let instruction_end = pc.checked_add(4).ok_or_else(|| {
                    ZkVmError::MemoryError("Integer overflow in instruction fetch".into())
                })?;

                if !segment.contains_address(instruction_end - 1)? {
                    return Err(ZkVmError::MemoryError(format!(
                        "Instruction at 0x{:x} crosses segment boundary",
                        pc
                    )));
                }
        ...
        Err(ZkVmError::MemoryError(format!(
            "Invalid instruction address: 0x{:x}",
            pc
        )))
    }
    ...
    fn decode_instruction(&self, instruction: u32) -> ZkVmResult<DecodedInstruction> {
        ...
        let instruction_type = match opcode {
            ...
            0x01 => InstructionType::Load {
                reg: reg1,
                addr: immediate as u32,
            },
            0x02 => InstructionType::Store {
                reg: reg1,
                addr: immediate as u32,
            },
            0x03 => InstructionType::Add {
                dest: reg1,
                src1: reg2,
                src2: reg3,
            },
            0x04 => InstructionType::Sub {
                dest: reg1,
                src1: reg2,
                src2: reg3,
            },
            0x05 => InstructionType::Jump {
                addr: immediate as u32,
            },
            0x06 => InstructionType::JumpIf {
                condition: reg1,
                addr: immediate as u32,
            },
    ...
    fn execute_instruction(&mut self, instruction: DecodedInstruction) -> ZkVmResult<()> {
        match instruction.instruction_type {
            ...
            InstructionType::Halt => {
                return Err(ZkVmError::ExecutionError(
                    "Halt instruction executed".into(),
                ));
            }
        }
        ...
        if let Some((_, segment)) = address_space.segments.range(..=addr_u64).next_back() {
            if segment.contains_address(addr_u64)? {
                ...
                if !segment.permissions.read {
                    return Err(ZkVmError::MemoryError(format!(
                        "No read permission at address 0x{:x}",
                        addr
                    )));
                }

                // Ensure we can read a full 4-byte value without overflow
                let read_end = addr_u64.checked_add(4).ok_or_else(|| {
                    ZkVmError::MemoryError("Integer overflow in memory read".into())
                })?;

                if !segment.contains_address(read_end - 1)? {
                    return Err(ZkVmError::MemoryError(format!(
                        "Read at 0x{:x} crosses segment boundary",
                        addr
                    )));
                }
        ...
        if let Some((_, segment)) = address_space.segments.range(..=addr_u64).next_back() {
            if segment.contains_address(addr_u64)? {
                ...
                if !segment.permissions.write {
                    return Err(ZkVmError::MemoryError(format!(
                        "No write permission at address 0x{:x}",
                        addr
                    )));
                }

                // Ensure we can write a full 4-byte value without overflow
                let write_end = addr_u64.checked_add(4).ok_or_else(|| {
                    ZkVmError::MemoryError("Integer overflow in memory write".into())
                })?;

                if !segment.contains_address(write_end - 1)? {
                    return Err(ZkVmError::MemoryError(format!(
                        "Write at 0x{:x} crosses segment boundary",
                        addr
                    )));
                }
    ...
    fn check_limits(&self) -> ZkVmResult<()> {
        ...
        if pc > MAX_ADDRESS {
            return Err(ZkVmError::ExecutionError(format!(
                "Program counter exceeds maximum address: 0x{:x}",
                pc
            )));
        }
        ...
        Err(ZkVmError::ExecutionError(format!(
            "Program counter out of bounds or no execute permission: 0x{:x}",
            pc
        )))
    }
    ...
    pub fn validate_memory_integrity(&self) -> ZkVmResult<()> {
        ...
        let calculated_total: usize = address_space
            .segments
            .values()
            .map(|seg| seg.size)
    ...
    pub fn deallocate_segment(&mut self, base: u64) -> ZkVmResult<()> {
        ...
        if let Some(segment) = address_space.segments.remove(&base) {
            ...
            address_space.allocated = address_space
                .allocated
                .checked_sub(segment.size)
                .ok_or_else(|| {
                    ZkVmError::MemoryError("Integer underflow in memory deallocation".into())
                })?;
            Ok(())
        } else {
            Err(ZkVmError::MemoryError(format!(
                "No segment found at base address 0x{:x}",
                base
            )))
        }
    }
...
mod tests {
    ...
    fn test_permission_enforcement() {
        ...
        let read_only = PagePermissions {
            read: true,
            write: false,
            execute: false,
        };
        let execute_only = PagePermissions {
            read: false,
            write: false,
            execute: true,
        };
        let no_perms = PagePermissions {
            read: false,
            write: false,
            execute: false,
        };
    ...
    fn test_segment_overlap_detection_comprehensive() {
        ...
        let seg6 = MemorySegment::new(
            0x10000 + MIN_SEGMENT_SIZE as u64,
            MIN_SEGMENT_SIZE,
            PagePermissions {
                read: true,
                write: false,
