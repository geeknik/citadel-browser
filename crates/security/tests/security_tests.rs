...
use citadel_security::{
    ...
    FingerprintProtectionLevel, SecurityContext, SecurityContextBuilder, SecurityError,
    SecurityMetrics, SecurityResult, SecuritySeverity, SecurityViolation, UrlScheme,
};
...
mod csp_tests {
    ...
    fn test_csp_hash_parsing() {
        ...
        let script_src = csp.directives.get(&CspDirective::ScriptSrc).unwrap();
        assert!(script_src.contains(&CspSource::Hash(
            "sha256".to_string(),
...
mod fingerprint_protection_tests {
    ...
    fn test_fingerprint_protection_configuration() {
        ...
        assert_eq!(
            current_protection.canvas_noise,
            custom_protection.canvas_noise
        );
        assert_eq!(
            current_protection.normalize_navigator,
            custom_protection.normalize_navigator
        );
        assert_eq!(
            current_protection.spoof_webgl,
            custom_protection.spoof_webgl
        );
    }
}
...
mod url_scheme_tests {
    ...
    fn test_url_scheme_validation() {
        ...
        assert!(context
            .validate_url_scheme("data:text/plain;base64,SGVsbG8gV29ybGQ=")
            .is_ok());
...
mod security_error_tests {
    ...
    fn test_security_error_severity() {
        ...
        // Low severity errors
        let invalid_config = SecurityError::InvalidConfiguration("Invalid setting".to_string());
    ...
    fn test_security_error_descriptions() {
        ...
        assert!(memory_exhaustion
            .remediation_advice()
            .contains("memory limits"));

        let sandbox_violation = SecurityError::SandboxViolation {
            operation: "network access".to_string(),
        };
        assert_eq!(sandbox_violation.short_description(), "sandbox_violation");
        assert!(sandbox_violation
            .remediation_advice()
            .contains("investigate immediately"));
    }
}
