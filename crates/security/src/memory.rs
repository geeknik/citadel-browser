...
impl ResourcePool {
    ...
    pub fn allocate(
        ...
        debug!(
            "Allocated {} bytes (ID: {}), pool memory: {} / {}",
    ...
    pub fn deallocate(&mut self, allocation_id: usize) -> MemoryProtectionResult<()> {
        if let Some(allocation) = self.allocations.remove(&allocation_id) {
            ...
            debug!(
                "Deallocated {} bytes (ID: {}), pool memory: {} / {}",
...
impl AttackDetector {
    ...
    fn detect_patterns(&self, current_size: usize) -> Option<AttackPattern> {
        ...
        if total_in_window > 200 * 1024 * 1024 {
            // 200MB in time window
...
impl MemoryProtectionSystem {
    ...
    pub fn allocate(
        ...
        if let Some(pool_mutex) = pools.get(&resource_type) {
            ...
            if pool.would_exceed_soft_limit(size) {
                info!(
                    "Soft limit approached for {}, triggering cleanup",
                    resource_type
                );
            ...
            match pool.allocate(size, critical, source) {
                Ok(allocation_id) => {
                    ...
                    *self.total_memory.lock().unwrap() += size;
                    ...
                    debug!(
                        "Successfully allocated {} bytes for {} (ID: {})",
                    ...
                    if pool.config.emergency_cleanup_enabled {
                        ...
                        if cleaned > 0 {
                            ...
                            match pool.allocate(size, critical, None) {
                                ...
                                Err(retry_error) => {
                                    error!(
                                        "Allocation failed even after emergency cleanup: {}",
                                        retry_error
                                    );
                                }
                            }
            ...
            match pool.deallocate(allocation_id) {
                Ok(()) => {
                    ...
                    *total_memory = total_memory.saturating_sub(size);
                    ...
                    debug!(
                        "Successfully deallocated {} bytes for {} (ID: {})",
                        size, resource_type, allocation_id
                    );
    ...
    pub fn force_cleanup(&self, aggressive: bool) -> usize {
        ...
        info!(
            "Force cleanup completed, freed {} bytes total",
            total_cleaned
        );
        total_cleaned
    }
    ...
    pub fn periodic_check(&self) {
        ...
        *last_check = now;
        ...
        debug!(
            "Memory utilization: {:.1}% ({} / {} bytes)",
            utilization * 100.0,
            total_memory,
            self.config.total_memory_limit
        );

        // Check if we need cleanup
        if utilization > self.config.emergency_threshold {
            error!(
                "Emergency memory threshold exceeded: {:.1}%",
                utilization * 100.0
            );
            *self.emergency_mode.lock().unwrap() = true;
            ...
            warn!(
                "Aggressive cleanup threshold exceeded: {:.1}%",
                utilization * 100.0
            );
...
mod tests {
    ...
    fn test_emergency_mode() {
        ...
        let _id = system
            .allocate(ResourceType::GenericMemory, 850, false, None)
            .unwrap();
