...
use citadel_zkvm::{Channel, ChannelMessage, ZkVm};
...
pub use ui::{Message as TabMessage, TabBar};
...
pub use zkvm_renderer::{
    render_in_isolation, DisplayItem, DisplayKind, RenderRequest, RenderedContent, SecurityMetadata,
};
...
impl Tab {
    ...
    pub async fn convert_to_container(&self) -> TabResult<()> {
        ...
        match state.tab_type {
            ...
            TabType::Container { .. } => Err(TabError::InvalidOperation(
    ...
    pub async fn load_url(&self, url: String) -> TabResult<()> {
        ...
        self.channel
            .send(ChannelMessage::ResourceRequest {
                url,
                headers: vec![],
            })
            .await?;
...
mod tests {
    ...
    fn test_page_content_variants() {
        let loading = PageContent::Loading {
            url: "https://example.com".to_string(),
        };
