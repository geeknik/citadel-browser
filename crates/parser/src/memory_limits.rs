...
impl ParserResourceTracker {
    ...
    pub fn check_parsing_timeout(&self) -> ParserResult<()> {
        if let Some(start_time) = self.parse_start_time {
            ...
            if elapsed > self.limits.max_parsing_time {
                ...
                return Err(crate::error::ParserError::ResourceLimitExceeded(format!(
                    "Parsing timeout exceeded: {} seconds",
                    elapsed
                )));
            }
        }
    ...
    pub fn track_dom_node(&self, size_bytes: usize) -> ParserResult<()> {
        ...
        *node_count += 1;
        ...
        if *node_count > self.limits.max_dom_nodes {
            error!(
                "DOM node limit exceeded: {} > {}",
                *node_count, self.limits.max_dom_nodes
            );
            self.metrics.increment_violations();
            return Err(crate::error::ParserError::ResourceLimitExceeded(format!(
                "DOM node limit exceeded: {}",
                self.limits.max_dom_nodes
            )));
        }
        ...
        debug!(
            "Tracked DOM node creation: {} nodes, {} bytes",
            *node_count, size_bytes
        );
    ...
    pub fn track_dom_depth(&self, depth: usize) -> ParserResult<()> {
        ...
        if *current_depth > self.limits.max_dom_depth {
            error!(
                "DOM depth limit exceeded: {} > {}",
                *current_depth, self.limits.max_dom_depth
            );
            self.metrics.increment_violations();
            return Err(crate::error::ParserError::ResourceLimitExceeded(format!(
                "DOM depth limit exceeded: {}",
                self.limits.max_dom_depth
            )));
        }
    ...
    pub fn track_css_rule(
        &self,
        selector_complexity: usize,
        size_bytes: usize,
    ) -> ParserResult<()> {
        ...
        *rule_count += 1;
        ...
        if *rule_count > self.limits.max_css_rules {
            error!(
                ...
                *rule_count, self.limits.max_css_rules
            );
            self.metrics.increment_violations();
            return Err(crate::error::ParserError::ResourceLimitExceeded(format!(
                "CSS rule limit exceeded: {}",
                self.limits.max_css_rules
            )));
        }
        ...
        if selector_complexity > self.limits.max_css_selector_complexity {
            error!(
                "CSS selector complexity exceeded: {} > {}",
        ...
        debug!(
            "Tracked CSS rule creation: {} rules, complexity {}, {} bytes",
    ...
    pub fn track_memory_allocation(&self, size_bytes: usize) -> ParserResult<()> {
        ...
        if size_bytes > self.limits.max_element_size {
            error!(
                ...
                size_bytes, self.limits.max_element_size
            );
            self.metrics.increment_violations();
            return Err(crate::error::ParserError::ResourceLimitExceeded(format!(
                "Element too large: {} bytes",
                size_bytes
            )));
        }
        ...
        *memory_usage += size_bytes;
        ...
        if *memory_usage > self.limits.max_parsing_memory {
            error!(
                "Parsing memory limit exceeded: {} > {}",
                *memory_usage, self.limits.max_parsing_memory
            );
            self.metrics.increment_violations();
            return Err(crate::error::ParserError::ResourceLimitExceeded(format!(
                "Parsing memory limit exceeded: {} bytes",
                self.limits.max_parsing_memory
            )));
        }

        debug!(
            "Tracked memory allocation: {} bytes (total: {} bytes)",
    ...
    pub fn track_js_memory(&self, heap_size: usize) -> ParserResult<()> {
        if heap_size > self.limits.max_js_heap_size {
            error!(
                "JavaScript heap size limit exceeded: {} > {}",
                heap_size, self.limits.max_js_heap_size
            );
            self.metrics.increment_violations();
            return Err(crate::error::ParserError::ResourceLimitExceeded(format!(
                "JavaScript heap too large: {} bytes",
                heap_size
            )));
        }
    ...
    pub fn get_utilization_ratios(&self) -> ParserUtilization {
        ...
        ParserUtilization {
            ...
            parsing_time: usage.parsing_time.unwrap_or(0) as f32
                / (self.limits.max_parsing_time * 1000) as f32,
        }
    }
}
...
impl ParserAttackDetector {
    ...
    pub fn detect_rapid_element_creation(&mut self, element_count: usize) -> bool {
        ...
        self.recent_elements
            .retain(|(timestamp, _)| now.duration_since(*timestamp).as_secs() < 1);
        ...
        if total_elements > self.rapid_element_threshold {
            warn!(
                "Rapid element creation attack detected: {} elements in 1 second",
                total_elements
            );
            return true;
        }
    ...
    pub fn detect_memory_bomb(&mut self, allocation_size: usize) -> bool {
        ...
        if total_allocations > self.memory_bomb_threshold {
            warn!(
                "Memory bomb attack detected: {} bytes allocated in 1 second",
                total_allocations
            );
            return true;
        }
