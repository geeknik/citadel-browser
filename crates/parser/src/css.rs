...
use cssparser::{Parser as CssParserImpl, ToCss, Token};
...
impl CitadelCssParser {
    ...
    pub fn new(config: ParserConfig, metrics: Arc<ParserMetrics>) -> Self {
        let security_context = Arc::new(SecurityContext::new(match config.security_level {
    ...
    fn parse_rule(&self, parser: &mut CssParserImpl) -> ParserResult<StyleRule> {
            ...
            match parser.next() {
                Ok(Token::CurlyBracketBlock) => {
                    ...
                    let declarations = parser
                        .parse_nested_block(|block_parser| {
                            ...
                            Ok(self
                                .parse_declarations_simple(&decl_text)
                                .unwrap_or_default())
                        })
                        .map_err(|e: cssparser::ParseError<()>| {
                            ParserError::CssError(format!("Error parsing declarations: {:?}", e))
                        })?;
                ...
                Err(e) => {
                    return Err(ParserError::CssError(format!(
                        "Error parsing selector: {:?}",
                        e
                    )));
                }
            }
        ...
        Err(ParserError::CssError(
            "Unexpected end of input while parsing rule".to_string(),
        ))
    }
...
impl CitadelStylesheet {
    ...
    fn apply_declaration(&self, computed: &mut ComputedStyle, declaration: &Declaration) {
        match declaration.property.as_str() {
            ...
            _ => {
                ...
                {
                    tracing::debug!(
                        "Vendor-prefixed CSS property ignored: {}",
    ...
    pub fn apply_shorthand_spacing(
        &self,
        ...
        while let Some(start) = value[current_pos..].find(|c: char| c.is_alphabetic()) {
            ...
            if let Some(paren_start) = value[actual_start..].find('(') {
                ...
                if let Some(paren_end) = value[paren_pos..].find(')') {
                    ...
                    if let Some(transform_fn) =
                        self.parse_single_transform_function(function_name, function_args)
                    {
            ...
            "translate" => {
                if args.len() >= 2 {
                    if let (Some(x), Some(y)) = (
                        self.parse_length_value(args[0]),
                        self.parse_length_value(args[1]),
                    ) {
...
mod tests {
    ...
    fn test_style_computation_with_taffy() {
        ...
        assert_eq!(
            highlight_styles.background_color,
            Some(ColorValue::Named("yellow".to_string()))
        ); // Yellow
        assert_eq!(highlight_styles.display, DisplayType::Flex);
        assert_eq!(
            highlight_styles.layout_style.flex_direction,
            FlexDirection::Row
        );
    ...
    fn test_color_parsing() {
        ...
        // Test named colors
        assert_eq!(
            stylesheet.parse_color_value("red"),
            Some(ColorValue::Named("red".to_string()))
        );
        assert_eq!(
            stylesheet.parse_color_value("blue"),
            Some(ColorValue::Named("blue".to_string()))
        );
        assert_eq!(
            stylesheet.parse_color_value("transparent"),
            Some(ColorValue::Named("transparent".to_string()))
        );

        // Test hex colors
        assert_eq!(
            stylesheet.parse_color_value("#ff0000"),
            Some(ColorValue::Hex("ff0000".to_string()))
        );
        assert_eq!(
            stylesheet.parse_color_value("#00ff00"),
            Some(ColorValue::Hex("00ff00".to_string()))
        );
        assert_eq!(
            stylesheet.parse_color_value("#0000ff"),
            Some(ColorValue::Hex("0000ff".to_string()))
        );
    }
    ...
    fn test_length_parsing() {
        ...
        assert_eq!(
            stylesheet.parse_length_value("16px"),
            Some(LengthValue::Px(16.0))
        );
        assert_eq!(
            stylesheet.parse_length_value("2em"),
            Some(LengthValue::Em(2.0))
        );
        assert_eq!(
            stylesheet.parse_length_value("20"),
            Some(LengthValue::Px(20.0))
        ); // Unitless
    }
    ...
    fn test_selector_matching() {
        ...
        assert!(stylesheet.selector_matches(
            ".highlight",
            "span",
            &["highlight".to_string()],
            None
    ...
    fn test_specificity_calculation() {
        ...
        assert!(
            parser.calculate_specificity("div.highlight") > parser.calculate_specificity("div")
        );
    }
}
