...
use taffy::{
    AvailableSpace, Dimension, Display, LengthPercentage, LengthPercentageAuto, NodeId, Position,
...
impl CitadelLayoutEngine {
    ...
    pub fn compute_layout(
        ...
        self.performance_monitor.cache_hit_ratio = self.performance_monitor.cache_hit_ratio * 0.9; // Decrease for cache miss
        ...
        if !needs_full_rebuild {
            ...
            if let Ok(incremental_result) =
                self.compute_incremental_layout(dom, stylesheet, &viewport_size)
            {
        ...
        let layout_result =
            self.compute_full_layout(dom, stylesheet, viewport_size.clone(), start_time)?;
    ...
    fn compute_incremental_layout(
        ...
        if self.dirty_tracker.dirty_nodes.is_empty() {
            return Err(ParserError::LayoutError(
                "No incremental update needed".to_string(),
            ));
        }
        ...
        for &dirty_node_id in &self.dirty_tracker.dirty_nodes {
            if let Some(&taffy_node) = self.node_map.get(&dirty_node_id) {
                ...
                self.taffy
                    ...
                    .map_err(|e| {
                        ParserError::LayoutError(format!("Incremental layout error: {:?}", e))
                    })?;
            }
        }
    ...
    fn build_node_recursive(
        ...
        if computed_style.display == DisplayType::None {
            ...
            let taffy_node = self
                ...
                .map_err(|e| {
                    ParserError::LayoutError(format!("Failed to create Taffy node: {:?}", e))
                })?;
            ...
            if let Some(estimated_bounds) = self.estimate_node_bounds(dom_node, stylesheet) {
                ...
                if !self.intersects_viewport(&estimated_bounds, &viewport_size) {
                    ...
                    let taffy_node = self
                        ...
                        .map_err(|e| {
                            ParserError::LayoutError(format!(
                                "Failed to create culled node: {:?}",
                                e
                            ))
                        })?;
        ...
        let taffy_node = if layout_children.is_empty() {
            ...
            self.taffy.new_leaf(measured_style).map_err(|e| {
                ParserError::LayoutError(format!("Failed to create leaf node: {:?}", e))
            })?
        } else {
            ...
            self.taffy
                ...
                .map_err(|e| {
                    ParserError::LayoutError(format!("Failed to create parent node: {:?}", e))
                })?
        };
    ...
    fn apply_grid_properties(&self, style: &mut Style, computed: &ComputedStyle) {
        ...
        // Grid gap properties
        if let Some(gap) = computed
            .grid_gap
            .as_ref()
            .or(computed.grid_row_gap.as_ref())
        {
        ...
        {
            let row_gap_value =
                self.convert_to_pixels(row_gap, &self.text_measurement.base_font_size);
            let col_gap_value =
                self.convert_to_pixels(col_gap, &self.text_measurement.base_font_size);
            style.gap = taffy::Size {
    ...
    fn convert_dimension(&self, length: &Option<LengthValue>) -> Dimension {
        match length {
            Some(length_val) => {
                let px_value =
                    self.convert_to_pixels(length_val, &self.text_measurement.base_font_size);
                match length_val {
    ...
    fn convert_dimension_auto(&self, length: &Option<LengthValue>) -> LengthPercentageAuto {
        match length {
            Some(length_val) => match length_val {
    ...
    fn convert_length_percentage_from_border_width(
        &self,
        border_width: &Option<LengthValue>,
    ) -> LengthPercentage {
        match border_width {
            Some(length_val) => match length_val {
                LengthValue::Percent(pct) => LengthPercentage::Percent(*pct / 100.0),
        ...
        match length {
            ...
            LengthValue::Vmin(vmin) => {
                let min_dimension = self
                    .viewport_context
                    .width
                    .min(self.viewport_context.height);
            ...
            LengthValue::Vmax(vmax) => {
                let max_dimension = self
                    .viewport_context
                    .width
                    .max(self.viewport_context.height);
            ...
            width += self
                ...
                .unwrap_or_else(|| {
                    ...
                    match ch {
                        ...
                        'i' | 'l' | 'I' | '1' | '!' | '|' => {
                            self.text_measurement.base_font_size * 0.3
                        }
    ...
    fn estimate_memory_usage(&self) -> usize {
        ...
        let node_map_size =
            self.node_map.len() * (std::mem::size_of::<u32>() + std::mem::size_of::<NodeId>());
        let taffy_map_size =
            self.taffy_map.len() * (std::mem::size_of::<NodeId>() + std::mem::size_of::<u32>());
        let text_measurement_size = std::mem::size_of::<TextMeasurement>()
            + self.text_measurement.char_widths.len()
                * (std::mem::size_of::<char>() + std::mem::size_of::<f32>());
...
impl CitadelLayoutEngine {
    ...
    pub fn update_viewport_size(&mut self, new_size: LayoutSize) -> ParserResult<()> {
        if let Ok(root_node) = self.get_root_node() {
            ...
            self.taffy
                .compute_layout(root_node, available_space)
                .map_err(|e| {
                    ParserError::LayoutError(format!("Failed to update layout: {:?}", e))
                })?;
        }
    ...
    pub fn update_zoom_factor(&mut self, zoom_factor: f32) -> ParserResult<()> {
        ...
        if let Ok(root_node) = self.get_root_node() {
            ...
            self.taffy
                .compute_layout(root_node, available_space)
                .map_err(|e| {
                    ParserError::LayoutError(format!("Failed to update layout for zoom: {:?}", e))
                })?
        }
    ...
    fn check_security_limits(&self, node_count: usize) -> ParserResult<()> {
        ...
        if node_count > max_nodes {
            return Err(ParserError::SecurityViolation(format!(
                "Layout tree too large: {} nodes (max: {})",
    ...
    fn hash_node_recursive(&self, node: &Node, hasher: &mut DefaultHasher) {
        ...
        for child_handle in node.children().iter().take(50) {
            // Limit to prevent deep recursion
    ...
    pub fn get_performance_stats(&self) -> PerformanceStats {
        let avg_layout_time = if self.performance_monitor.layout_count > 0 {
            self.performance_monitor.total_layout_time as f64
                / self.performance_monitor.layout_count as f64
        } else {
            0.0
        };
...
mod tests {
    use super::*;
    use crate::css::{Declaration, StyleRule};
    use crate::dom::*;
    use crate::security::SecurityContext;
    ...
    fn test_viewport_resize() {
        ...
        layout_engine
            .compute_layout(&dom, &stylesheet, initial_size)
            .unwrap();
    ...
    fn test_comprehensive_css_property_mapping() {
        ...
        assert_eq!(
            ...
            Some(crate::css::LengthValue::Px(20.0))
        );
    ...
    fn test_css_shorthand_properties() {
        ...
        stylesheet.apply_shorthand_spacing(
            &mut computed,
            "10px 20px 30px 40px",
            crate::css::SpacingType::Margin,
        );
        ...
        assert_eq!(
            ...
            Some(crate::css::LengthValue::Px(30.0))
        );
        assert_eq!(
            computed.margin_left,
            Some(crate::css::LengthValue::Px(40.0))
        );
        ...
        stylesheet.apply_shorthand_spacing(
            &mut computed2,
            "15px 25px",
            crate::css::SpacingType::Padding,
        );

        assert_eq!(
            computed2.padding_top,
            Some(crate::css::LengthValue::Px(15.0))
        );
        assert_eq!(
            computed2.padding_right,
            Some(crate::css::LengthValue::Px(25.0))
        );
        assert_eq!(
            computed2.padding_bottom,
            Some(crate::css::LengthValue::Px(15.0))
        );
        assert_eq!(
            computed2.padding_left,
            Some(crate::css::LengthValue::Px(25.0))
        );
    }
