use citadel_networking::{resource::ResourceType, ResourceManager};
...
impl ResourceLoader {
    ...
    pub async fn load_resource(&self, url: Url) -> Result<Vec<u8>, String> {
        ...
        match self
            .resource_manager
            .fetch(url.as_str(), Some(ResourceType::Other))
            .await
        {
            Ok(response) => {
                log::info!(
                    "Successfully fetched resource: {} (status: {}, {} bytes)",
                    url,
                    response.status(),
                    response.body().len()
                );
    ...
    pub async fn load_html(&self, url: Url) -> Result<String, String> {
        match self.resource_manager.fetch_html(url.as_str()).await {
            Ok(response) => {
                log::info!(
                    "Successfully fetched HTML: {} (status: {}, {} bytes)",
                    url,
                    response.status(),
                    response.body().len()
                );
    ...
    pub async fn load_css(&self, url: Url) -> Result<String, String> {
        match self
            .resource_manager
            .fetch(url.as_str(), Some(ResourceType::Css))
            .await
        {
            Ok(response) => {
                log::info!(
                    "Successfully fetched CSS: {} (status: {}, {} bytes)",
                    url,
                    response.status(),
                    response.body().len()
                );
    ...
    pub async fn load_javascript(&self, url: Url) -> Result<String, String> {
        match self
            .resource_manager
            .fetch(url.as_str(), Some(ResourceType::Script))
            .await
        {
            Ok(response) => {
                log::info!(
                    "Successfully fetched JavaScript: {} (status: {}, {} bytes)",
                    url,
                    response.status(),
                    response.body().len()
                );
    ...
    pub async fn load_image(&self, url: Url) -> Result<Vec<u8>, String> {
        match self
            .resource_manager
            .fetch(url.as_str(), Some(ResourceType::Image))
            .await
        {
            Ok(response) => {
                log::info!(
                    "Successfully fetched image: {} (status: {}, {} bytes)",
                    url,
                    response.status(),
                    response.body().len()
                );
