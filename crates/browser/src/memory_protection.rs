...
use crate::performance::{CleanupPriority, PerformanceMonitor};
use citadel_security::{
    MemoryProtectionBuilder, MemoryProtectionConfig, MemoryProtectionSystem, ResourceType,
    SecurityContext,
};
...
impl BrowserMemoryManager {
    ...
    pub fn new(
        ...
        let mut protection_system = MemoryProtectionBuilder::new()
            ...
            .thresholds(
                protection_config.aggressive_threshold,
                protection_config.emergency_threshold,
            )
    ...
    fn start_background_monitoring(
        ...
        task::spawn(async move {
            ...
            loop {
                ...
                if memory_usage > 0 {
                    debug!(
                        "Memory status: {:.1}MB ({:.1}% utilization)",
    ...
    pub fn deallocate_memory(
        ...
        match self
            .protection_system
            .deallocate(resource_type.clone(), allocation_id)
        {
            Ok(()) => {
                ...
                self.performance_monitor
                    .update_memory_usage(component, self.get_resource_memory_usage(resource_type));
    ...
    pub fn force_cleanup(&self, aggressive: bool) -> usize {
        info!(
            "Forcing browser memory cleanup (aggressive: {})",
            aggressive
        );
        ...
        let cleanup_priority = if aggressive {
    ...
    fn update_all_performance_metrics(&self) {
        ...
        for (resource_type, stat) in stats {
            ...
            self.performance_monitor
                .update_memory_usage(component, stat.current_memory);
        }
    }
    ...
    pub fn get_memory_statistics(&self) -> BrowserMemoryStatistics {
        ...
        BrowserMemoryStatistics {
            ...
            dom_memory: stats
                .get(&ResourceType::DomNodes)
                .map(|s| s.current_memory)
                .unwrap_or(0),
            js_memory: stats
                .get(&ResourceType::JsObjects)
                .map(|s| s.current_memory)
                .unwrap_or(0),
            network_memory: stats
                .get(&ResourceType::NetworkConnections)
                .map(|s| s.current_memory)
                .unwrap_or(0),
            image_memory: stats
                .get(&ResourceType::ImageData)
                .map(|s| s.current_memory)
                .unwrap_or(0),
            css_memory: stats
                .get(&ResourceType::CssRules)
                .map(|s| s.current_memory)
                .unwrap_or(0),
    ...
    pub fn can_create_tab(&self) -> bool {
        ...
        if current_utilization > tab_limit_ratio {
            warn!(
                "Rejecting new tab creation due to high memory usage: {:.1}%",
    ...
    pub fn emergency_browser_cleanup(&self) -> usize {
        ...
        if self.is_emergency_mode() {
            ...
            self.performance_monitor
                .add_measurement("emergency_cleanup", 1);
...
pub struct BrowserMemoryStatistics {
    ...
    pub resource_stats:
        std::collections::HashMap<ResourceType, citadel_security::ResourcePoolStats>,
}
...
mod tests {
    use super::*;
    use crate::performance::{MemoryConfig, PerformanceMonitor};
    ...
    async fn test_dom_memory_allocation() {
        ...
        memory_manager
            .deallocate_memory(ResourceType::DomNodes, allocation_id)
            .unwrap();
    }
