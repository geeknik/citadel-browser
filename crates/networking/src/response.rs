...
impl Response {
    ...
    pub fn body_text(&self) -> Result<String, NetworkError> {
        String::from_utf8(self.body.to_vec()).map_err(|_| {
            NetworkError::ResourceError("Failed to convert response body to string".to_string())
        })
    }

    /// Get the response body as JSON
    pub fn json<T: serde::de::DeserializeOwned>(&self) -> Result<T, NetworkError> {
        serde_json::from_slice(&self.body).map_err(NetworkError::SerializationError)
    }
    ...
    pub fn is_script(&self) -> bool {
        self.content_type()
            .map(|ct| {
                ...
                ct.contains("javascript") || ct.contains("application/js") || ct.contains("text/js")
            })
            .unwrap_or(false)
    }
