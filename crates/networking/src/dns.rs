...
impl CitadelDnsResolver {
    ...
    pub async fn with_tracker_blocking(
        mode: DnsMode,
        tracker_blocker: Arc<TrackerBlockingEngine>,
    ) -> Result<Self, NetworkError> {
        log::info!(
            "🔧 Creating DNS resolver with tracker blocking and mode: {:?}",
            mode
        );
    ...
    async fn resolve_with_doh(
        ...
        if url.scheme() != "https" {
            return Err(NetworkError::DnsError(
                "DoH URLs must use HTTPS".to_string(),
            ));
        }
        ...
        if all_addresses.is_empty() {
            return Err(NetworkError::DnsError(format!(
                "No records found for {} via DoH",
                hostname
            )));
        }

        log::debug!(
            "🔒 DoH resolved {} to {} addresses",
            hostname,
            all_addresses.len()
        );
        Ok(all_addresses)
            ...
            for answer in answers {
                if let Some(answer_type) = answer["type"].as_u64() {
                    if let Some(data) = answer["data"].as_str() {
                        ...
                        if (record_type == "A" && answer_type == 1)
                            || (record_type == "AAAA" && answer_type == 28)
                        {
    ...
    async fn should_block_hostname_advanced(
        &self,
        hostname: &str,
    ) -> Option<crate::tracker_blocking::BlockedRequest> {
...
mod tests {
    ...
    async fn test_local_cache_resolver() {
        let resolver = CitadelDnsResolver::new()
            .await
            .expect("DNS resolver creation should succeed in tests");
    ...
    async fn test_doh_resolution() {
        let doh_url = "https://cloudflare-dns.com/dns-query";
        let resolver = CitadelDnsResolver::with_mode(DnsMode::DoH(doh_url.to_string()))
            .await
            .expect("DoH resolver creation should succeed");
        ...
        match resolver.resolve("example.com").await {
            Ok(addresses) => {
                assert!(
                    !addresses.is_empty(),
                    "DoH should resolve to at least one address"
                );
