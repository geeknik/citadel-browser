                        ...
                        Ok(resolved_url) => {
                            ...
                            if self.is_safe_url(&resolved_url) {
                                let mut resource_ref =
                                    ResourceRef::new(resolved_url, *resource_type)
                                        .with_critical(*is_critical);
...
mod tests {
    ...
    fn test_html_resource_discovery() {
        ...
        let css_found = resources
            .iter()
            .any(|r| r.url.path().ends_with("style.css") && r.resource_type == ResourceType::Css);
        let js_found = resources.iter().any(|r| {
            r.url.path().ends_with("script.js") && r.resource_type == ResourceType::Script
        });
        let img_found = resources
            .iter()
            .any(|r| r.url.path().ends_with("image.png") && r.resource_type == ResourceType::Image);
    ...
    fn test_css_resource_discovery() {
        ...
        let has_css = resources
            .iter()
            .any(|r| r.resource_type == ResourceType::Css);
        let has_font = resources
            .iter()
            .any(|r| r.resource_type == ResourceType::Font);
    ...
    fn test_resource_context_filtering() {
        ...
        let context = ResourceContext::new(base_url.clone()).include_non_critical(false);
        ...
        let context = ResourceContext::new(base_url).allowed_types(vec![ResourceType::Css]);
        ...
        assert!(resources
            .iter()
            .all(|r| r.resource_type == ResourceType::Css));
    }
    ...
    fn test_url_resolution() {
        ...
        // Test absolute URL
        let absolute_url = discovery
            .resolve_url("https://cdn.example.com/font.woff", &base_url)
            .unwrap();
        assert_eq!(absolute_url.as_str(), "https://cdn.example.com/font.woff");

        // Test protocol-relative URL
        let protocol_relative = discovery
            .resolve_url("//cdn.example.com/script.js", &base_url)
            .unwrap();
        assert_eq!(
