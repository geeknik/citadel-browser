...
impl Request {
    ...
    pub fn new(method: Method, url: &str) -> Result<Self, NetworkError> {
        ...
        if url.scheme() != "https" && url.scheme() != "data" && url.scheme() != "about" {
            return Err(NetworkError::HttpsEnforcementError(format!(
                "Non-HTTPS URL: {}. HTTPS is required for privacy and security.",
                url
            )));
        }
    ...
    fn apply_maximum_privacy(&mut self) {
        ...
        self.headers
            .insert("User-Agent".to_string(), Self::generate_random_user_agent());
    ...
    fn apply_high_privacy(&mut self) {
        ...
        self.headers.insert(
            ...
            Self::generate_generic_user_agent(),
        );
    ...
    fn apply_balanced_privacy(&mut self) {
        ...
        if !self.headers.contains_key("User-Agent") {
            self.headers.insert(
                "User-Agent".to_string(),
                Self::generate_standard_user_agent(),
            );
        }
    }
