...
use citadel_networking::{
    CitadelDnsResolver, DnsMode, Method, NetworkConfig, PrivacyLevel, Request,
};
...
async fn test_dns_resolver_creation() {
    ...
    assert!(
        resolver.is_ok(),
        "DNS resolver should initialize successfully"
    );
}
                    ...
                    match ip {
                        ...
                        std::net::IpAddr::V6(ipv6) => {
                            ...
                            if segments[0] & 0xfe00 == 0xfc00 {
                                println!(
                                    "Warning: {} resolved to unique local IPv6: {}",
                                    domain, ipv6
                                );
                            }
                        }
...
fn test_request_creation_validation() {
    ...
    for url in valid_urls {
        ...
        assert!(
            result.is_ok(),
            "Valid HTTPS URL {} should create request successfully",
            url
        );
    }
...
fn test_request_body_limits() {
    ...
    match request.body() {
        Some(body) => {
            assert!(
                body.len() <= 100_000_000, // 100MB max
...
fn test_network_config_security() {
    ...
    assert!(
        config.randomize_user_agent,
        "User agent should be randomized"
    );
    assert!(
        config.strip_tracking_params,
        ...
        match Url::parse(url_str) {
            Ok(url) => {
                ...
                if let Some(host) = url.host_str() {
                    ...
                    assert!(
                        !host.contains(".."),
                        "Host should not contain directory traversal"
                    );
                }
            }
            Err(e) => {
...
async fn test_concurrent_dns_resolution() {
    ...
    let domains = vec!["example.com", "google.com", "github.com", "rust-lang.org"];
