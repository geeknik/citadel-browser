...
async fn test_system_dns_resolver_creation() {
    ...
    assert!(
        resolver.is_ok(),
        "Should be able to create system DNS resolver"
    );
...
async fn test_doh_providers() {
    ...
    for provider in &providers {
        ...
        assert!(
            url.is_ok(),
            "DoH provider URL should be valid: {}",
            provider
        );
        assert_eq!(
            url.unwrap().scheme(),
            "https",
            "DoH provider should use HTTPS: {}",
            provider
        );
    }
    ...
    assert!(
        providers.contains(&random_provider),
        "Random provider should be from the known list"
    );
}
...
async fn test_dns_resolution_with_different_modes() {
    ...
    match system_resolver.resolve("example.com").await {
        Ok(addresses) => {
            assert!(
                !addresses.is_empty(),
                "Should resolve example.com to at least one address"
            );
            println!(
                "System DNS resolved example.com to {} addresses",
                addresses.len()
            );
        ...
        Err(e) => {
            ...
            println!(
                "System DNS resolution failed (might be expected in CI): {}",
                e
            );
        }
    }
...
async fn test_doh_resolution() {
    ...
    let doh_resolver =
        CitadelDnsResolver::with_mode(DnsMode::DoH(DohProviders::CLOUDFLARE.to_string()))
            .await
            .unwrap();
    ...
    match doh_resolver.resolve("example.com").await {
        Ok(addresses) => {
            assert!(
                !addresses.is_empty(),
                "DoH should resolve example.com to at least one address"
            );
...
async fn test_dns_cache_functionality() {
    ...
    if let Ok(_) = resolver.resolve("example.com").await {
        ...
        assert_eq!(
            stats_after_first.cache_entries, 1,
            "Should have one cache entry after first resolution"
        );
        ...
        if let Ok(_) = resolver.resolve("example.com").await {
            ...
            assert_eq!(
                ...
                "Should have one cache hit after second resolution"
            );
        }
    }
}
...
async fn test_tracker_blocking() {
    ...
    for domain in &tracking_domains {
        ...
        if let Err(e) = result {
            assert!(
                ...
                "Error should indicate domain was blocked: {}",
                e
            );
        }
    }
    ...
    assert!(
        stats.queries_blocked > 0,
        "Should have blocked some queries"
    );
}
...
async fn test_invalid_hostnames() {
    ...
    for hostname in &invalid_hostnames {
        ...
        assert!(
            result.is_err(),
            "Invalid hostname {} should fail validation",
            hostname
        );
    }
}
...
async fn test_dns_mode_switching() {
    ...
    // Switch to DoH mode
    let doh_result = resolver
        .set_mode(DnsMode::DoH(DohProviders::CLOUDFLARE.to_string()))
        .await;
    ...
    // Switch to DoT mode
    let dot_result = resolver
        .set_mode(DnsMode::DoT("1.1.1.1:853".to_string()))
        .await;
    ...
    assert!(
        local_result.is_ok(),
        "Should be able to switch back to LocalCache mode"
    );
    assert_eq!(resolver.get_mode(), DnsMode::LocalCache);

    // Test invalid DoH URL
    let invalid_result = resolver
        .set_mode(DnsMode::DoH("not-a-url".to_string()))
        .await;
...
async fn test_concurrent_dns_resolution() {
    ...
    let mut tasks: Vec<
        tokio::task::JoinHandle<Result<Vec<std::net::IpAddr>, citadel_networking::NetworkError>>,
    > = Vec::new();
    ...
    println!(
        "Concurrent DNS resolution: {}/{} succeeded",
        success_count,
        results.len()
    );
